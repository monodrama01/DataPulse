  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <span className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>/</span>
              <span className={`${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'} font-medium`}>1.1 重复数据自动去除</span>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className={`text-3xl font-bold mb-6 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>重复数据自动去除</h1>
        <div className={`${theme === 'dark' ? 'bg-white/5' : 'bg-white'} rounded-lg shadow-sm p-8 space-y-4`}>
          <ul className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'} list-disc pl-6 space-y-2`}>
            <li>重复类型：完全重复、部分重复、近似重复</li>
            <li>去重策略：全字段/关键字段/相似度（模糊匹配）</li>
            <li>工具：pandas.drop_duplicates、SQL DISTINCT/窗口函数、Excel 透视表</li>
            <li>注意：去重前备份、与业务确认</li>
          </ul>
          <pre className={`${theme === 'dark' ? 'bg-gray-900/60 text-gray-100' : 'bg-gray-50 text-gray-800'} p-4 rounded overflow-x-auto`}>
<code>{`# pandas 基于关键字段去重
df = df.drop_duplicates(subset=["user_id", "mobile"], keep="last")

-- SQL DISTINCT 示例
SELECT DISTINCT user_id, mobile FROM users;`}</code>
          </pre>
        </div>
      </div>
    </div>
