...
export default function ModelSelectionOptimizationPage() {
  ...
  return (
    <div className={`min-h-screen ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <span className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'}`}>/</span>
              <span className={`${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'} font-medium`}>2.2 模型选择与优化</span>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className={`text-3xl font-bold mb-6 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>模型选择与优化</h1>
        <div className={`${theme === 'dark' ? 'bg-white/5' : 'bg-white'} rounded-lg shadow-sm p-8 space-y-4`}>
          <ul className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'} list-disc pl-6 space-y-2`}>
            <li>模型类别：线性/树/神经网络/集成</li>
            <li>自动策略：问题类型、数据特征、性能权衡、交叉验证</li>
            <li>比较：准确率/精确率/召回/F1/AUC、统计检验、可视化</li>
            <li>AutoML：Auto-sklearn、H2O、Google AutoML</li>
          </ul>
        </div>
      </div>
    </div>
