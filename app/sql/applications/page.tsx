...
export default function SQLApplicationsPage() {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className={`${theme === 'dark' ? 'bg-white/5' : 'bg-white'} rounded-lg shadow-sm p-8 space-y-6`}>
          <section>
            ...
            <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>生成 RFM 基础数据，筛选高价值用户。</p>
          </section>
          <section>
            <h2 className={`text-xl font-semibold mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>应用2：商品销售趋势分析</h2>
            <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>按月统计销量并排名，评估热销品。</p>
          </section>
          <section>
            <h2 className={`text-xl font-semibold mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>应用3：渠道效果分析</h2>
            <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>评估各渠道获客与变现能力，计算单用户产值。</p>
          </section>
          <section>
            <h2 className={`text-xl font-semibold mb-2 ${theme === 'dark' ? 'text-gray-100' : 'text-gray-900'}`}>应用4：窗口函数实战</h2>
            <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>月度各地区销售额排名与累计占比。</p>
          </section>
        </div>
      </div>
    </div>
